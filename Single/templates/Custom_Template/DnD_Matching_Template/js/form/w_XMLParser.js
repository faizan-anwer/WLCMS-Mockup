function setXml(){
	...
	for (var i=0; i<optionsNode.childNodes.length; i++)
	{
		...
		REMOVE_CDATA(optNode.children[0]);
		REMOVE_CDATA(optNode.children[1]);
		REMOVE_CDATA(optNode.children[2]);
		REMOVE_CDATA(optNode.children[3]);
		optNode.children[3].setAttribute("name","");
		optNode.children[3].setAttribute("ver","");
		REMOVE_CDATA(optNode.children[4]);
		REMOVE_CDATA(optNode.children[5]);
		optNode.children[5].setAttribute("name","");
		optNode.children[5].setAttribute("ver","");
	}
	...
	$options.children().each(function(i,elem){
		if($(this).attr("class") != "hide")
		{
			...
			SET_CDATA(xmlDoc,optNode.children[0],dropText);
			SET_CDATA(xmlDoc,optNode.children[1],draggableObject);
			SET_CDATA(xmlDoc,optNode.children[2],feedback);
			
			SET_CDATA(xmlDoc,optNode.children[3],feedbackAudio.path);
			optNode.children[3].setAttribute("name",feedbackAudio.name);
			optNode.children[3].setAttribute("ver",feedbackAudio.ver);
			
			SET_CDATA(xmlDoc,optNode.children[4],hint);
			
			SET_CDATA(xmlDoc,optNode.children[5],hintAudio.path);
			optNode.children[5].setAttribute("name",hintAudio.name);
			optNode.children[5].setAttribute("ver",hintAudio.ver);
