function setXml(){
	...
	TEMPLATE.VALIDATE.MSG.BLANK = "<div class='error'>There should be at least two 'Draggable Objects' to proceed.</div>";
	var divClass = TEMPLATE.VALIDATE.MSG_DIV;
	TEMPLATE.VALIDATE.MSG_DIV = ".errPrepend_0";
	if(TEMPLATE.VALIDATE.MATCH($options.parent().parent(),$options.children().length,"0|1|2",$options.parent().parent().parent().parent().parent().parent().parent().parent()))
	{
		errors.push(0);
	}
	...
	$options.children().each(function(i,elem){
		...
		if($this.attr("class") != "hide")
		{
			...
			SET_CDATA(xmlDoc,optNode.children[0],draggableObject);
			optNode.children[1].setAttribute("columnListId",correctCategory);
			SET_CDATA(xmlDoc,optNode.children[2],correctFeedback);
			
			SET_CDATA(xmlDoc,optNode.children[3],correctFeedbackAudio.path);
			optNode.children[3].setAttribute("name",correctFeedbackAudio.name);
			optNode.children[3].setAttribute("ver",correctFeedbackAudio.ver);
			
			SET_CDATA(xmlDoc,optNode.children[4],incorrectFeedback);
			
			SET_CDATA(xmlDoc,optNode.children[5],incorrectFeedbackAudio.path);
			optNode.children[5].setAttribute("name",incorrectFeedbackAudio.name);
			optNode.children[5].setAttribute("ver",incorrectFeedbackAudio.ver);
