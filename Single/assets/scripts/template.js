var TEMPLATE = function () {
    ...
    return {
		...
		DYNAMIC_MODAL: function(style,trg,htmlPage,type,icon_id,_limit){
			...
			TEMPLATE.MODAL_VIEW($mElm.find(".modal-content"),"../../Modals/"+htmlPage+".html");
			$mElm.modal('show');
		},
		//	END DYNAMIC MODAL
		...
		TEXT_MODAL: function(trg){
			...
			var $mElm = $("#dynamicModal");
			TEMPLATE.MODAL_VIEW($mElm.find(".modal-content"),"../../Modals/TextEditor.html");
			$mElm.modal('show');
		},
		//	END TEXT MODAL
		...
		CONFIRM_DELETE_ROW: function (confirm){
			...
			if(confirm)
			{
				...
				if(TEMPLATE.CACHE._table.children().length > TEMPLATE.CACHE._limit || typeof TEMPLATE.CACHE._limit == "undefined")
				{
					...
					//console.log($btns.find(".find").html() + " - " + $btns.find(".upload").html());
				}
				//	End - Enhance Version ***************************************
